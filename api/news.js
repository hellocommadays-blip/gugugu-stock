...
function parseRSSItems(xml, sourceName) {
  ...
  for (const block of itemBlocks.slice(0, 15)) {// api/news.js — 產業新聞分析
// 抓 Google News RSS → 篩選重點新聞 → Claude 分析影響鏈 + 對照自選股
...
以下是用戶的自選股清單（共 ${watchlistSymbols.length} 檔）：
${watchlistText}

請嚴格按照以下格式，逐一列出上方清單中的每一檔自選股（不可省略任何一檔）：

股票代號 股票名稱：今日新聞中是否有相關？如果有，說明影響方向（50字內）；如果完全無關，請說「今日無直接相關新聞」。

注意事項：
- 必須逐一列出清單中的每一檔，一行一檔，不能跳過任何一檔
- 清單已在上方明確提供，不要說「未提供自選股清單」
- 不要分析清單以外的股票
- 語氣中性客觀，不給買賣建議
- 禁止使用 #、|、---、Markdown表格語法`;
